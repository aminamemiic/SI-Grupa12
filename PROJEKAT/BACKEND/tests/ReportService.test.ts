...
describe("ReportService feature contract", () => {
  let app: any;
  ...
  test("treba podrzati generisanje izvjestaja sa periodom", async () => {
    const report = {
      ...
      breakdowns: {
        byMonth: [{ label: "Maj 2026", total: 1500, count: 2, average: 750 }],
      },
      expenses: [],
    };
  ...
  test("treba podrzati export detaljnog izvjestaja", async () => {
    mockReportService.exportExpenseReport.mockResolvedValue({
      buffer: Buffer.from("Detaljni izvjestaj troskova"),
